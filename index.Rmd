...
```{r plot1, class.source="code-r-small", message=FALSE, echo=FALSE, warning=FALSE}
p1 <- ggplot(sb_locs_state, aes(x=population, y=n_stores, color=state)) +
  geom_point(alpha=0.8)
```

```{r plot2, class.source="code-r-small", message=FALSE, echo=FALSE, warning=FALSE}
p2 <- ggplot(sb_nutr, aes(x=Calories, fill=Category)) +
  geom_histogram(data=subset(sb_nutr,Category == 'Drinks'), alpha=0.5, binwidth = 50) +
  geom_histogram(data=subset(sb_nutr,Category == 'Food'), alpha=0.5, binwidth = 50)
```
